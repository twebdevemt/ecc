...
async function runTests() {
  ...
  if (await asyncTest('creates compaction log', async () => {
                ...
                for (const entry of hookArray) {
                    for (const hook of entry.hooks) {
                        if (hook.type === "command") {
                            assert.ok(hook.command.startsWith("node"), `Hook command should start with 'node': ${hook.command.substring(0, 50)}...`);
                        }
                    }
                }
            };
    ...
    if (
        test("script references use CLAUDE_PLUGIN_ROOT variable", () => {
            const hooksPath = path.join(__dirname, "..", "..", "hooks", "hooks.json");
            const hooks = JSON.parse(fs.readFileSync(hooksPath, "utf8"));
    ...
    if (
        test("plugin.json does NOT have explicit hooks declaration", () => {
            ...
            // See: https://github.com/twebdevemt/ecc/issues/103
